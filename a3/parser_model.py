...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings, 0, w.reshape(-1)).reshape(4, -1)
    ...
    def forward(self, w):
        ...
        x = self.embedding_lookup(w).reshape(-1, 1, self.embed_size * self.n_features)
        x = F.relu((x @ self.embed_to_hidden_weight).reshape(-1, self.hidden_size) + self.embed_to_hidden_bias)
        x = self.dropout(x)
        logits = (x.reshape(-1, 1, self.hidden_size) @ self.hidden_to_logits_weight).reshape(-1, self.n_classes)
