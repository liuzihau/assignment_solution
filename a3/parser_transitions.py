...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack = ['ROOT']
        self.buffer = [word for word in sentence] if sentence else []
        self.dependencies = []
...
def minibatch_parse(sentences, model, batch_size):
    ...
    unfinished_parses = partial_parses[:]  #copy.copy(partial_parses)
    while unfinished_parses:
        step_name = model.predict(unfinished_parses[:batch_size])
        for step, parser in zip(step_name, unfinished_parses[:batch_size]):
            parser.parse_step(step)
